...
\newcommand{\slateWatermark}[1]{%
  \usebackgroundtemplate{%
    \tikz[remember picture, overlay]{%
      \node[opacity=0.067, anchor=south east, inner sep=0]
...
\newcommand{\slatetitlepage}{%
  \begin{tikzpicture}[remember picture, overlay]
    ...
    % opacity=0.90. At full strength the photograph and the title box sit at
    % the same weight and the box stops reading as something laid over it.
    % Ten per cent is enough to settle it, and no more than that: by 0.80 the
    % autumn colour has gone grey and the cover looks faded rather than quiet.
    % Change the one number.
    \node[above right, inner sep=0pt, opacity=0.90]
      at (current page.south west)
      {\includegraphics[width=\paperwidth]{\slateBackground}};
