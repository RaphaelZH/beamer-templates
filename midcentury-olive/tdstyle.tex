...
%
% VERSION MATTERS, AND THE VERSION CHANGED. This is now the Duffner/Pardo
% EBGaramond12 continuation — one optical size, weights 400 to 800, with a real
% Bold and a real Bold Italic. It replaces Duffner's 0.016, which shipped two
% optical sizes and NO BOLD in any of them.
%
% 0.016 was defensible while it lasted: an old-style serif used for display was
% never bolded, and \bfseries on a face with no bold makes fontspec synthesise
% one by smearing the outline, which on a Garamond looks like a printing fault.
% So the title fonts were pinned to \mdseries and BoldFont was mapped onto
% *-Regular, and nothing ever asked for a weight that did not exist.
%
% They are still pinned to \mdseries below. That is the design — Garamond
% titles are set at medium weight here — and it is now a choice rather than a
% constraint. What changed is that \bfseries on this face resolves to a drawn
% Bold instead of quietly resolving to Regular.
%
% A WARNING ABOUT 0.016, IF YOU EVER GO BACK. That release looks like it has a
% bold and does not. Its 08 optical master is 41% thicker in the stem than its
% 12 at a matched cap height — the same order as a genuine Medium-to-Bold step —
% so the two sit side by side in a file listing looking exactly like a weight
% pair. They are not. Two measurements separate them: a real bold gets WIDER as
% it gets heavier (this family +0.9%, Montserrat +2.7%) while 08 gets NARROWER
% than 12 by 1.5%, because it is drawn to be set at 8pt, tight and inky; and a
% real weight pair holds its x-height-to-cap-height ratio constant, while 0.016
% shifts it from 0.631 at size 12 to 0.616 at size 08. Using 08 as a bold gives
% emphasis that is heavier, narrower and differently proportioned — a different
% font, which is the one thing emphasis must never look like.
% -----------------------------------------------------------------------------
\IfFileExists{fonts/EBGaramond/otf/EBGaramond-Regular.otf}{}{%
  \PackageError{tdstyle}{%
    EB Garamond not found in fonts/EBGaramond/otf/.\MessageBreak
    Get it (SIL OFL) from\MessageBreak
    https://github.com/octaviopardo/EBGaramond12 —\MessageBreak
    at least Regular, Italic, Bold, BoldItalic as .otf%
  }{See the DISPLAY FACE section of tdstyle.tex.}%
}

\newfontfamily\mcmDisplayFont{EBGaramond}[
  Path = fonts/EBGaramond/otf/, Extension = .otf,
...
\newfontfamily\mcmDisplayFontLS{EBGaramond}[
  Path = fonts/EBGaramond/otf/, Extension = .otf,
...
% -----------------------------------------------------------------------------
\newfontfamily\mcmTitleFontSC{EBGaramond}[
  Path = fonts/EBGaramond/otf/, Extension = .otf,
  UprightFont = *-Regular, BoldFont = *-Bold,
...
% ON. All four bundled faces report `smcp` — checked rather than assumed, and
% worth re-checking on any face you swap in, because a family that ships its
% small caps as a SEPARATE font instead of as a feature will make \textsc and
% Letters=SmallCaps fail silently, setting ordinary lowercase with no warning.
% slate-blocks/ hit exactly that with Cormorant.
%
%   python3 -c "from fontTools.ttLib import TTFont; \
%     f=TTFont('fonts/EBGaramond/otf/EBGaramond-Regular.otf'); \
